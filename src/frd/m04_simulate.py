...
def profiles_needed(election_rules_list):
    ...
    orders = not set(election_rules_set).isdisjoint(ORDINAL_RULES) #bool
    cardinals = not set(election_rules_set).isdisjoint(CARDINAL_RULES) #bool
    return approvals, orders, cardinals
...
def run_simulation(n_iter, profile_param_vals, election_param_vals, del_voting_param_vals):
    ...
    for it in range(n_iter):
        for profile_params in helper.params_dict_to_tuples(profile_param_vals):
            # create profile
            # compute voter majority
