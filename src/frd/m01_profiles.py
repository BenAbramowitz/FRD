...
class Profile():
    def __init__(self, n_voters:int, n_cands:int, n_issues:int, voters_p, cands_p, approval_params:Tuple[int,float]=None):
        ...
        self.vote_majority_outcomes = None
    ...
    def get_voter_majority(self):
        self.vote_majority_outcomes = np.sum(self.v_pref, axis=0) > self.n_voters
    ...
    def get_voter_majority(self):
        return self.vote_majority_outcomes
    ...
    def new_instance(self, approvals = True, ordinals = True, agreements = True, seed=None):
        ...
        self.issues_to_distances()
        if approvals: self.distances_to_approvals()
        if ordinals: self.distances_to_ordinals()
        if agreements: self.distances_to_agreements()
        #return vars(self)
